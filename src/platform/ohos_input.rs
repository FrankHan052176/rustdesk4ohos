...
pub(super) fn inject_pointer(kind: &str, mask: i32, x: i32, y: i32, display_id: u64) {
    if !is_authorized() {
        return;
    }
    ...
    match result {
        Ok(()) => report_input_success(kind),
        Err(error) => report_input_error(&format!("{kind} event: {error}")),
    }
}
...
pub(super) fn inject_key(event: &KeyEvent, display_id: u64) {
    if !is_authorized() {
        return;
    }
    ...
    let result = if event.press {
        inject_key_action(key_code, KEY_ACTION_DOWN, display_id)
            .and_then(|_| inject_key_action(key_code, KEY_ACTION_UP, display_id))
    } else {
        inject_key_action(
            key_code,
            if event.down {
    ...
    match result {
        Ok(()) => report_input_success("key"),
        Err(error) => report_input_error(&error),
    }
}

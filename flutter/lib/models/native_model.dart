...
class PlatformFFI {
  ...
  Timer? _ohosHostClipboardTimer;
  bool _ohosHostClipboardSyncing = false;
  String? _ohosHostClipboardText;
  ...
  Future<String> startOhosHost() async {
    ...
    try {
      await _toOhosChannel.invokeMethod<bool>('ensureMicrophonePermission');
    } catch (error) {
      debugPrint('Failed to request HarmonyOS microphone permission: $error');
    }
    var clipboardEnabled = false;
    try {
      clipboardEnabled = await _toOhosChannel
              .invokeMethod<bool>('ensureClipboardPermission') ??
          false;
    } catch (error) {
      debugPrint('Failed to request HarmonyOS clipboard permission: $error');
    }
    await _ffiBind.mainSetOhosHostClipboardEnabled(enabled: clipboardEnabled);
    if (clipboardEnabled) {
      try {
        _ohosHostClipboardText =
            await _toOhosChannel.invokeMethod<String>('getClipboardText') ?? '';
        await _ffiBind.mainUpdateOhosHostClipboardText(
            text: _ohosHostClipboardText!);
      } catch (error) {
        clipboardEnabled = false;
        await _ffiBind.mainSetOhosHostClipboardEnabled(enabled: false);
        debugPrint('Failed to initialize HarmonyOS clipboard sync: $error');
      }
    }
  ...
  void _startOhosHostClipboardSync() {
    _ohosHostClipboardTimer?.cancel();
    _ohosHostClipboardTimer = Timer.periodic(
      const Duration(milliseconds: 250),
      (_) => _syncOhosHostClipboard(),
    );
  }

  void _stopOhosHostClipboardSync() {
    _ohosHostClipboardTimer?.cancel();
    _ohosHostClipboardTimer = null;
    _ohosHostClipboardSyncing = false;
    _ohosHostClipboardText = null;
  }

  Future<void> _syncOhosHostClipboard() async {
    if (_ohosHostClipboardSyncing) return;
    _ohosHostClipboardSyncing = true;
    try {
      final local =
          await _toOhosChannel.invokeMethod<String>('getClipboardText') ?? '';
      if (local != _ohosHostClipboardText) {
        _ohosHostClipboardText = local;
        await _ffiBind.mainUpdateOhosHostClipboardText(text: local);
      }
      final remote = await _ffiBind.mainTakeOhosHostClipboardText();
      if (remote != null && remote != _ohosHostClipboardText) {
        _ohosHostClipboardText = remote;
        await _toOhosChannel
            .invokeMethod<void>('setClipboardText', {'text': remote});
      }
    } catch (error) {
      debugPrint('HarmonyOS host clipboard sync failed: $error');
    } finally {
      _ohosHostClipboardSyncing = false;
    }
  }
